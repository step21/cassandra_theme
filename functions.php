...
if ( ! function_exists( 'cassandra_setup' ) ) :
	...
	function cassandra_setup() {
		 ...
		 * If you're building a theme based on cassandra, use a find and replace
		 * to change 'cassandra' to the name of your theme in all the template files.
		 */
		load_theme_textdomain( 'cassandra', get_template_directory() . '/languages' );
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'cassandra' ),
			)
		);
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'cassandra_custom_background_args',
				array(
...
function cassandra_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'cassandra_content_width', 640 );
}
...
function cassandra_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'cassandra' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'cassandra' ),
...
function cassandra_scripts() {
	wp_enqueue_style( 'cassandra-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'cassandra-style', 'rtl', 'replace' );

	wp_enqueue_script( 'cassandra-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
	wp_enqueue_script( 'cassandra-js', get_template_directory_uri() . '/js/cassandra.js', array('jquery'), _S_VERSION, true );
